...
import numpy as np
import os
os.environ['CUDA_VISIBLE_DEVICES'] = '-1' # Paksa CPU-only di baris paling atas
...
def load_model_and_map():
    print("Memuat model dan peta (hanya sekali)...")
    model = tf.keras.models.load_model(MODEL_PATH, compile=False)
    coral_map = gpd.read_file(SHAPEFILE_PATH)
    return model, coral_map

@st.cache_data(ttl=3600) # Cache data selama 1 jam
def get_latest_data_and_mask(_today_str, _coral_map_obj):
    ...
    start_date = end_date - timedelta(days=TIME_STEPS)
...
# PERBAIKAN: Buat fungsi terpisah untuk prediksi dan cache hasilnya
@st.cache_data(ttl=3600) # Cache hasil prediksi selama 1 jam
def get_prediction(input_data):
    """Membuat prediksi dan menyimpan hasilnya di cache."""
    print("Membuat prediksi baru (proses ini lambat di CPU)...")
    input_for_model = np.expand_dims(input_data, axis=0)
    # Gunakan model yang sudah dimuat di scope global
...
with st.spinner("Mengambil & memproses data satelit (bisa lambat saat pertama kali)..."):
    latest_data, reef_mask = get_latest_data_and_mask(today_str, coral_map)

# Panggil fungsi prediksi yang sudah di-cache
predicted_risk_map = get_prediction(latest_data)
...
results_df = pd.DataFrame({'latitude': lat_grid.flatten(),'longitude': lon_grid.flatten(),'risk_score': final_prediction.flatten()})
...
color_map = {'Sangat Rendah': '#3b4cc0', 'Rendah': '#5ac8c8', 'Sedang': '#f2e750','Tinggi': '#ff993e', 'Sangat Tinggi': '#e60000'}
st.info("Peta ini hanya menampilkan prediksi di lokasi terumbu karang yang teridentifikasi.")
fig = px.scatter_mapbox(results_df, lat="latitude", lon="longitude", color="Level Risiko", color_discrete_map=color_map, category_orders={"Level Risiko": labels}, mapbox_style="carto-positron", zoom=4, center=dict(lat=-2.5, lon=118), opacity=0.8, hover_name="risk_score", hover_data={"Level Risiko": True, "latitude": ":.4f", "longitude": ":.4f"}, labels={"Level Risiko": "Level Risiko"})
